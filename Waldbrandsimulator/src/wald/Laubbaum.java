		...
		if (!this.brennen)
			for (int i = 0; i < 12; i++) {
				if (this.entzundetvon[i] != null) {
					if (this.entzundetvon[i].getX() == w.x
							&& this.entzundetvon[i].getY() == w.y) {
						break;
					}
				} else {
	...
	public void feuer() {
		if (this.brennzeit == Laubbaum.brenndauer) {
			...
			Waldflaeche.wald.flaeche[this.x][this.y] = new Asche(x, y,
					this.zuendcounter, this.entzundetvon, false);
			// this.inbrannt();
			for (int i = 0; i < 12; i++) {
				if (this.entzundetvon[i] != null) {
		...
		try {
			Waldflaeche.wald.flaeche[this.x + 1][this.y + 1].entzuenden(this);
		} catch (ArrayIndexOutOfBoundsException e) {
		...
		try {
			Waldflaeche.wald.flaeche[this.x][this.y + 2].entzuenden(this);
		} catch (ArrayIndexOutOfBoundsException e) {
		...
		try {
			Waldflaeche.wald.flaeche[this.x + 2][this.y].entzuenden(this);
		} catch (ArrayIndexOutOfBoundsException e) {
