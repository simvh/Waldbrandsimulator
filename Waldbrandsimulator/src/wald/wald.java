	...
	public wald(String filename) throws FileNotFoundException {
		...
		for (int x = 0; x < i; x++) {
			...
			for (int y = 0; y < j; y++) {
				...
				switch (c) {
				case 'N':
					flaeche[x][y] = new Nadelbaum(x, y);
					break;
				case 'L':
					flaeche[x][y] = new Laubbaum(x, y);
					break;
				case '-':
					flaeche[x][y] = new Busch(x, y);
					this.Bäume--;
					break;
				case 'B':
					flaeche[x][y] = new Brand(x, y);
					this.Bäume--;
					break;
				case 'A':
					flaeche[x][y] = new Asche(x, y, 0);
					this.Bäume--;
					break;
				case 'a':
					flaeche[x][y] = new abgeholzt(x, y);
					break;
				}
			}
		}
	...
	public wald(wald w) {
		...
		for (int x = 0; x < i; x++) {
			for (int y = 0; y < j; y++) {
				...
				case "N":
					flaeche[x][y] = new Nadelbaum(x, y);
					flaeche[x][y].brennen = w.flaeche[x][y].brennen;
					flaeche[x][y].brennzeit = w.flaeche[x][y].brennzeit;
					flaeche[x][y].zuendcounter = w.flaeche[x][y].zuendcounter;
					break;
				case "L":
					flaeche[x][y] = new Laubbaum(x, y);
					flaeche[x][y].brennen = w.flaeche[x][y].brennen;
